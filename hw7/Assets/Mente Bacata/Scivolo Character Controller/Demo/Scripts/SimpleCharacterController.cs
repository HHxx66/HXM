    ...
    public class SimpleCharacterController : MonoBehaviour
    {
        ...
        private void UpdateMovement(Vector3 moveDirection, float deltaTime)
        {
            ...
            if (isGrounded)
            {
                ...
                verticalSpeed = 0f;
                velocity += GroundClampSpeed * hero.transform.up;
            ...
            else
            {
                ...
                velocity += verticalSpeed * hero.transform.up;
            }
        ...
        {
            Vector3 forward = Vector3.ProjectOnPlane(cameraTransform.forward, hero.transform.up).normalized;
            Vector3 right = Vector3.Cross(hero.transform.up, forward);
        ...
        {
            Vector3 direzioneOrizz = Vector3.ProjectOnPlane(direction, hero.transform.up);

            if (direzioneOrizz.sqrMagnitude < 1E-06f)
                return;

            Quaternion rotazioneObbiettivo = Quaternion.LookRotation(direzioneOrizz, hero.transform.up);
            hero.transform.rotation = Quaternion.RotateTowards(hero.transform.rotation, rotazioneObbiettivo, rotationSpeed * Time.deltaTime);
        }
        ...
        private PlatformDisplacement GetPlatformDisplacementAtPoint(MovingPlatform platform, Vector3 point)
        {
            ...
            angle *= Mathf.Sign(Vector3.Dot(axis, hero.transform.up));
        ...
        private void BounceDownIfTouchedCeiling()
        {
            ...
            {
                if (Vector3.Dot(moveContacts[i].normal, hero.transform.up) < -0.7f)
                {
        ...
        {
            hero.transform.Translate(platformDisplacement.deltaPosition, Space.World);
            hero.transform.Rotate(0f, platformDisplacement.deltaUpRotation, 0f, Space.Self);
        }
